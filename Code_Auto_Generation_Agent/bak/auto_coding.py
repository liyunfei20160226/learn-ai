...
def parse_args():
    ...
    parser.add_argument(
        '--architecture',
        help='Path to architecture.json file (defaults to <prd-path>.architecture.json)'
    )
    parser.add_argument(
        '--use-agent',
        action='store_true',
        help='Use ReAct Agent mode (tool calling, more stable but slightly slower)'
    )
...
def main():
    ...
    if args.architecture:
        ...
        # 默认路径：把prd路径的.architecture.json
        prd_dir = os.path.dirname(args.prd_path)
        prd_base = os.path.splitext(os.path.basename(args.prd_path))[0]
        architecture_path = os.path.join(prd_dir, f"{prd_base}.architecture.json")
        logger.info(f"未指定架构文档，尝试使用默认路径: {architecture_path}")
    ...
    if not args.target_dir:
        ...
        # 处理双重扩展名，如 .prd.json → 去掉 .prd.json
        # 先去掉最后一个扩展名
        dir_name = os.path.splitext(prd_basename)[0]
        # 如果还包含 .prd，也去掉（处理 xxx.prd.json 格式）
        if dir_name.endswith('.prd'):
            dir_name = dir_name[:-4]
        # 默认放在 output/ 目录下
