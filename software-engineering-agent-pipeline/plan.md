...
|:--:|:-------|:-----|:-----|:---------|:-------------|
| 1 | **需求分析** | 把用户模糊需求整理成标准化需求规格 | 用户原始需求 | `01-requirements-spec.md`<br/>`requirements.json` | 完整性<br/>清晰性<br/>可测试性<br/>无歧义 |
...
const requirementsChecklist = [
  { id: 'complete', question: '是否覆盖了用户所有需求？', severity: 'error' },
  { id: 'clear', question: '每个需求描述是否清晰无歧义？', severity: 'error' },
  { id: 'testable', question: '每个需求是否可测试验证？', severity: 'warning' },
  { id: 'prioritized', question: '需求是否区分了优先级？', severity: 'info' },
];
```
...
```typescript
function afterQualityGate(state): nextNode {
  if (gateResult.passed) {
    return 下一个阶段Agent;
  } else {
    return 跳转到 gateResult.targetStageId;
  }
}
```
...
### 方案：**Claude Skill 编排多个独立Agent**

```
software-engineering-agent-pipeline/
├── skills/
│   └── se-pipeline.skill.ts         # 主控Skill，控制流程流转
├── agents/
│   ├── requirements-agent.md        # 需求分析Agent提示词
│   ├── architecture-agent.md        # 架构设计Agent提示词
│   ├── ui-prototype-agent.md        # UI原型Agent提示词
│   ├── database-agent.md            # 数据库设计Agent提示词
│   ├── task-breakdown-agent.md      # 任务拆分Agent提示词
│   ├── codegen-agent.md             # 代码生成Agent提示词
│   ├── codereview-agent.md          # 代码评审Agent提示词
│   ├── testing-agent.md             # 测试验证Agent提示词
│   ├── pre-release-agent.md         # 预发布检查Agent提示词
│   └── deployment-agent.md          # 部署运维Agent提示词
├── quality-checks/
│   ├── requirements-checklist.txt   # 需求检查清单
│   ├── architecture-checklist.txt   # 架构检查清单
│   └── ...                          # 各阶段检查清单
└── src/
    ├── types.ts                     # 类型定义
    ├── pipeline-orchestrator.ts     # 流程编排逻辑
    ├── memory-client.ts             # memory-mcp客户端
    └── storage.ts                   # 项目和制品存储
```

**优势**：
- ✅ **轻量** - 不需要 heavy 框架，开发快
- ✅ **模块化** - 每个Agent独立，可以单独调用、单独修改
- ✅ **可复用** - 某个Agent可以拿出去单独用
- ✅ **贴合Claude Code生态** - Skill天生就是干这个的
- ✅ **调试方便** - 出了问题改某个Agent就行，不用动整体架构

### 核心架构

```mermaid
flowchart TD
    S[Claude Skill<br/>se-pipeline] -->|控制流程| A1[需求分析Agent]
    S -->|控制流程| A2[架构设计Agent]
    S -->|控制流程| A3[UI设计Agent]
    S -->|...| ...
...
**说明：**
- **Skill 主控** - 控制流程顺序、处理质量闸门判断、处理回流跳转、协调人工确认
- **独立Agent** - 每个阶段一个Agent，模块化，可独立调用，可复用
- **共享知识底座** - 所有Agent读写同一个知识图谱（复用 memory-mcp），保证一致性
- **本地文件存储** - 按项目分目录保存所有阶段制品
- **质量闸门** - 自动检查 + 人工确认，不通过就回流到目标Agent
...
│   │   ├── artifacts.ts           # 制品类型定义
│   │   ├── pipeline.ts            # 流水线状态类型
│   │   └── quality-gate.ts        # 质量闸门类型
│   ├── agents/
│   │   ├── index.ts               # 导出
│   │   ├── base-agent.ts          # Agent基类/接口
│   │   ├── requirements-agent.ts  # 需求分析
│   │   ├── architecture-agent.ts  # 架构设计
│   │   ├── ui-prototype-agent.ts  # UI原型设计
│   │   ├── database-agent.ts      # 数据库设计
│   │   ├── task-breakdown-agent.ts # 任务拆分
│   │   ├── codegen-agent.ts       # 代码生成
│   │   ├── codereview-agent.ts    # 代码评审
│   │   ├── testing-agent.ts       # 测试验证
│   │   ├── pre-release-agent.ts   # 预发布检查
│   │   └── deployment-agent.ts    # 部署运维
│   ├── prompts/
│   │   ├── requirements.md        # 需求分析提示词模板
│   │   ├── architecture.md        # 架构设计提示词模板
│   │   ├── ui-prototype.md        # UI原型提示词模板
│   │   ├── database.md            # 数据库设计提示词模板
│   │   ├── task-breakdown.md      # 任务拆分提示词模板
│   │   ├── codegen.md             # 代码生成提示词模板
│   │   ├── codereview.md          # 代码评审提示词模板
│   │   ├── testing.md             # 测试验证提示词模板
│   │   ├── pre-release.md         # 预发布检查提示词模板
│   │   ├── deployment.md          # 部署运维提示词模板
│   │   └── quality-check/
│   │       ├── requirements.txt   # 需求阶段检查清单
│   │       ├── architecture.txt   # 架构阶段检查清单
│   │       ├── ui-prototype.txt   # UI设计检查清单
│   │       ├── database.txt       # 数据库检查清单
│   │       ├── task-breakdown.txt # 任务拆分检查清单
│   │       ├── codegen.txt         # 代码生成检查清单
│   │       ├── codereview.txt     # 代码评审检查清单
│   │       ├── testing.txt        # 测试验证检查清单
│   │       ├── pre-release.txt    # 预发布检查清单
│   │       └── deployment.txt     # 部署检查清单
│   ├── orchestrator/
│   │   ├── index.ts              # 导出
│   │   └── pipeline-orchestrator.ts # 流程编排器（Skill核心逻辑）
│   ├── knowledge/
│   │   ├── index.ts              # 导出
│   │   └── memory-mcp-client.ts  # memory-mcp客户端封装
│   ├── quality-gate/
│   │   ├── index.ts              # 导出
│   │   ├── auto-reviewer.ts      # 自动评审实现
│   │   └── checklists.ts         # 检查清单定义
│   └── storage/
│       ├── index.ts              # 导出
│       └── project-store.ts       # 项目和制品存储（本地文件）
├── examples/
│   └── todo-app/                # 示例：待办应用完整输出
...
### 阶段一：MVP 最小可行验证 (预计 2-3 天)

**目标**：验证核心思路可行，Skill编排走通

**任务：**
- [ ] 创建项目骨架，TypeScript配置
- [ ] 定义所有核心类型（artifacts, pipeline, quality-gate）
- [ ] 实现 memory-mcp 客户端连接
- [ ] 实现项目存储（本地文件存储）
- [ ] 实现流程编排器核心逻辑（Skill主控）
- [ ] 实现前5个Agent + 提示词模板 + 质量检查清单
  - 需求分析Agent
  - 架构设计Agent
  - UI/设计原型Agent
  - 数据库设计Agent
  - 任务拆分Agent
- [ ] 实现自动质量闸门 + 回流机制
- [ ] 端到端测试：用一个简单需求（TODO应用）跑通完整流程

**验收标准**：
> 用户输入"做一个REST API待办清单应用"，Skill能自动按顺序调用Agent，每个阶段输出制品，质量闸门自动检查，需要人工确认时停下来等用户，发现问题能回流修改，最终输出完整文档。

---

### 阶段二：完整实现所有Agent (预计 2-3 天)
...
| 阶段 | Skill自动做 | 用户需要做 |
|------|------------|-----------|
| 1.需求分析 | Agent生成需求文档 → 自动质量检查 | ⏸ 停下来等待确认 → 看需求对不对，提修改意见 → 反复直到满意 ✓ |
...
2. 系统 → 需求分析Agent → 输出需求文档 → 自动质量闸门检查通过
3. ⏸ **暂停，等待用户确认需求
4. 用户："我还要加个搜索功能，分类要支持标签分类，去掉评论" → 填写反馈
5. 系统 → 回流到需求分析Agent → 重新生成需求 → 再次等待用户确认
6. 用户："好了，需求没问题了" → 确认通过
7. 系统 → 自动进入架构设计Agent → 输出架构设计 → 等待用户确认
8. 用户："技术选型换PostgreSQL，不要MySQL" → 回流修改
9. 用户："架构OK" → 确认通过
10. 系统 → 自动依次运行：UI设计 → 数据库设计 → 任务拆分
    (这些阶段技术细节，用户可以不用看，直接自动过)
11. 系统 → 代码生成 → 代码评审 → 测试 → 预发布检查 → 部署
12. 输出完整项目给用户
```
