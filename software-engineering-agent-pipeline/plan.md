...
| 序号 | Agent | 职责 | 输入 | 输出制品 | 检查要点 |
|:--:|:-------|:-----|:-----|:---------|:-------------|
| 1a | **需求分析师** | 通过交互式询问逐步澄清用户模糊需求，挖掘真实需求 | 用户原始需求 + 历史问答 | 扩展后的`qa-history.yaml`（新增问题和回答） | - |
| 1b | **需求验证官** | 独立重新阅读所有问答，验证需求是否足够清晰，若仍有疑问则要求继续提问 | 完整问答历史 | 扩展后的`qa-history.yaml`（追加追问问题） | 发现遗漏<br>发现歧义 |
| 1 | **需求生成** | 所有问题澄清后，整理生成最终标准化需求规格 | 完整问答历史（已验证通过） | `01-requirements-spec.md`<br>`requirements.yaml`<br>`qa-history.yaml` | 完整性<br>清晰性<br>可测试性<br>无歧义 |
...
系统采用**双LLM模型分离架构**，兼顾功能完整性和成本效率：

| 模型 | 职责 | 使用场景 | 配置方式 |
|------|------|----------|----------|
| **文本LLM** | 纯文本分析、问答、生成 | 需求分析、架构设计、代码生成等所有文本处理任务 | `OPENAI_MODEL` / `OPENAI_API_KEY` / `OPENAI_BASE_URL` |
| **Vision LLM** | 图片文字提取、文档OCR | 处理用户上传的图片格式参考资料（截图、草图、手写文档等） | `VISION_OPENAI_MODEL` / `VISION_OPENAI_API_KEY` / `VISION_OPENAI_BASE_URL` |

**设计特点：**
- 灵活配置：如果没有配置单独的Vision模型，自动复用文本LLM，降级兼容
- 成本优化：文本任务用低成本文本模型，仅OCR才调用多模态模型
- 支持不同API服务商：文本和Vision可以来自不同服务商
...
需求分析阶段**完全支持用户上传参考资料**：
- 支持格式：`Markdown`、`TXT`、`PDF`、图片（通过Vision OCR提取文字）
- 预处理机制：所有上传文档自动解析提取文字，生成**项目背景文档**供给分析师使用
- 增量变更支持：
  - **文档变更**：上传/删除文档后，`documents_processed` 标记自动重置，点击"重新分析"重新预处理所有文档并整合变更
  - **文字变更**：需求分析完成后，通过"需求变更"按钮直接输入文字补充说明，系统自动保存到问答历史并启动新一轮分析
- 分析师和验证官在每一轮提问都会看到**完整的项目背景 + 完整的问答历史**，保证增量变更正确整合
...
1. 用户上传文档 
    ↓
2. 文档预处理器按格式提取文字 
   (Markdown/TXT/PDF直接提取，图片通过Vision LLM OCR提取)
    ↓
3. 生成整合后的项目背景
    ↓
4. 设置 documents_processed = true
    ↓
5. 分析师使用项目背景进行分析
...
1. 用户删除/新增文档
    ↓
2. 自动设置 documents_processed = false
    ↓
3. 用户点击"重新分析"按钮
    ↓
4. 跳转到步骤 2 重新预处理所有文档
...
需求分析采用**双层循环验证**机制，确保需求被充分澄清：

```mermaid
flowchart LR
    START([开始]) --> A1[需求分析师<br>基于历史生成问题]
    A1 --> W{有未回答<br>问题?}
    W -- 有 --> W1[等待用户<br>回答问题] --> W
    W -- 无 --> C1{分析师<br>需要更多提问?}
    C1 -- 是 --> A1
    C1 -- 否 --> V[需求验证官<br>独立验证]
    
    V --> W2{有未回答<br>问题?}
    W2 -- 有 --> W3[等待用户<br>回答问题] --> W2
    W2 -- 无 --> C2{验证官<br>需要更多提问?}
    C2 -- 是 --> V
    C2 -- 否 --> F[需求生成Agent<br>输出最终需求规格] --> END([进入下一阶段])
    
    style START fill:#f0f8ff,stroke:#333,stroke-width:2px
    style END fill:#f0f8ff,stroke:#333,stroke-width:2px
    style A1 fill:#e6f3ff,stroke:#333,stroke-width:1px
    style V fill:#ffe6e6,stroke:#333,stroke-width:1px
```

**流程说明**：

1. **第一循环：需求分析师循环**
   - 分析师基于当前完整问答历史，判断是否还需要提问
   - 如果需要，生成问题 → 等待用户回答 → 分析师再次判断
   - **直到分析师自己认为所有关键问题都已澄清**，才进入验证阶段
   - 原则：一次只问 2-5 个问题，避免一次性抛给用户太多问题

2. **第二循环：需求验证官循环**（分析师满意后才开始）
   - 验证官以独立第三方视角重新检查完整问答历史
   - 如果发现遗漏或歧义，追加问题 → 等待用户回答 → 验证官再次检查
   - **验证官全程独立循环，不再回退给分析师**
   - 直到验证官也认为所有关键问题都已澄清，才进入需求生成阶段

**优势**：
- 分工清晰：分析师负责主动挖掘，验证官负责查漏补缺
- 双重验证：两次独立检查，降低遗漏关键信息的概率
- 用户友好：每次只问少量问题，体验更好
...
项目提供完整的 **FastAPI + HTMX + SSE** Web 交互界面，支持全流程可视化操作：

### 核心交互特性

| 功能 | 说明 |
|------|------|
| **项目管理** | 创建、删除项目，列表展示所有项目 |
| **参考文档上传** | 支持多文件上传，支持 Markdown、TXT、PDF、图片格式，图片通过 Vision LLM 自动 OCR 提取文字 |
| **实时进度推送** | 使用 SSE（Server-Sent Events）流式推送分析进度，前端实时显示当前节点和进度条 |
| **交互式问答** | 问题分批展示，用户回答后自动继续分析流程 |
| **需求规格预览下载** | 需求分析完成后可在线预览或下载 Markdown 格式需求文档 |

### 增量需求变更支持

需求分析完成后，**始终支持增量变更**，两种方式：

1. **文档变更**（原有方式）：
   - 上传新文档 / 删除现有文档 → 系统自动标记 `documents_processed = False`
   - 界面自动显示蓝色提示，并滚动到"重新分析"按钮
   - 点击"重新分析" → 重新预处理所有文档 → 分析师整合最新文档内容重新分析

2. **纯文字变更**（新增方式）：
   - 需求分析完成卡片上提供"需求变更"按钮
   - 点击后弹出输入框 → 用户直接输入一句话变更说明
   - 点击"提交变更" → 系统自动保存变更到问答历史 → **一步到位直接启动新一轮分析**，不需要额外点击
   - 分析师基于**完整历史问答 + 新增变更说明**重新分析，产出更新后的需求规格

### 需求变更后按钮状态

- 当需求分析已完成且文档未处理 → 开始按钮变为**橙色渐变**，文字显示"**重新分析**"
- 当分析正在进行中 → 按钮自动禁用防止重复点击
