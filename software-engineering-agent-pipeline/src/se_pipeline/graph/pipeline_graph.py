...
def build_requirements_internal_graph(
    ...
    def quality_gate_node(state: PipelineState) -> PipelineState:
        """质量闸门自动评审"""
        if state.requirements_spec is not None:
            ...
            if not result.passed:
                # 质量不通过，需要回流
                state.requirements_verification_passed = False
                state.needs_more_questions = True
        return state

    graph.add_node("analyst", analyst_node)
    graph.add_node("wait_user", lambda s: s)  # 占位节点，表示需要外部等待用户输入
